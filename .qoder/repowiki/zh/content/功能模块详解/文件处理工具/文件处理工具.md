...
## 更新摘要
**已修改内容**
- 在简介、项目结构、核心组件和架构总览等章节中添加了对文本对比工具的说明
- 新增了“文本对比工具”详细组件分析章节
- 更新了依赖关系分析，添加了diff库的引用
- 更新了目录以包含新章节
...
- 文本对比：基于diff库，支持两段文本的差异对比，支持文件上传、差异高亮显示与结果导出。
...
- 文本对比：双文本输入、差异对比、高亮显示、导航定位与结果导出。
...
- [src/pages/textDiff.tsx](file://src/pages/textDiff.tsx#L77-L459)

## 架构总览
浏览器端处理链路概览：
- 输入层：Dropzone/粘贴/选择文件
- 处理层：pdf-lib/docx/jsPDF/html2canvas/@ffmpeg/Canvas/diff
...
- [src/pages/textDiff.tsx](file://src/pages/textDiff.tsx#L107-L156)
...
### 文本对比工具（diff）
- 设计目标：对比两段文本的差异，支持高亮显示、导航定位与结果导出。
- 用户交互：输入或上传两段文本、开始对比、查看差异、导出结果。
- 关键实现要点：
  - 使用diff库的diffLines方法进行行级对比，生成差异结果。
  - 通过DiffItem和DiffNavItem数据结构管理差异项，支持行号定位。
  - 实现差异导航功能，点击导航项可平滑滚动到对应差异位置，并添加高亮动画。
  - 支持将对比结果导出为文本文件，包含差异统计和详细内容。
  - 使用useCallback优化性能，避免不必要的重新渲染。
- 错误处理：捕获对比异常，提示用户对比失败。

```mermaid
sequenceDiagram
participant U as "用户"
participant TD as "TextDiffTool"
participant DIFF as "diff库"
participant B as "Blob/URL"
participant DL as "下载"
U->>TD : 输入/上传两段文本
TD->>DIFF : 执行diffLines对比
DIFF-->>TD : 返回差异结果
TD->>TD : 生成差异导航与高亮显示
TD->>B : 生成导出文件Blob
TD->>DL : 触发下载
...
图表来源
- [src/pages/textDiff.tsx](file://src/pages/textDiff.tsx#L107-L156)
- [src/pages/textDiff.tsx](file://src/pages/textDiff.tsx#L171-L198)
- [src/pages/textDiff.tsx](file://src/pages/textDiff.tsx#L159-L168)
...
- [src/pages/textDiff.tsx](file://src/pages/textDiff.tsx#L1-L10)
...
- [src/pages/watermark.tsx](file://src/pages/watermark.tsx#L103-L191)
- [src/pages/fileUploader.tsx](file://src/pages/fileUploader.tsx#L20-L40)
- [src/pages/textDiff.tsx](file://src/pages/textDiff.tsx#L114-L154)
